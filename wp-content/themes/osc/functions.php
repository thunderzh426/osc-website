...
// Positions sidebar
	register_sidebar(array(
		...
		'id' => 'position-sidebar',
		'description' => 'Appears as the sidebar on the career page',
                ...
                <li>
                    ...
                    <div class="widget_content">
                        <h6><a href="<?php the_permalink() ?>"
                               title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><?php
                                $tit = the_title('', '', FALSE);
                                echo substr($tit, 0, 30);
                                if (strlen($tit) > 30)
                                    echo "...";
                                ?></a></h6>
                        <h6 class="title"><a href="<?php the_permalink() ?>" rel="bookmark"
...
// The widget form
        ...
        <p>
            ...
            <input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>"
                   type="text" value="<?php echo $title; ?>" class="widefat"/>
        </p>
        <p>
            <label
                for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of posts to show:', 'one-page'); ?></label>
...
function wpb_sender_email( $original_email_address ) {
    return 'info@oscgc.com';
}

// Function to change sender name
function wpb_sender_name( $original_email_from ) {
	return 'OSC Technologies';
}
...
function custom_woocommerce_page_title( $page_title ) {
  if( $page_title == 'Shop' ) {
    return "OSC Training Products";
  }
}
add_filter( 'woocommerce_page_title', 'custom_woocommerce_page_title');
function rfvc_update_order_status( $order_id, $status ) {
 
 $order = new WC_Order( $order_id );
 
  echo '<script type="text/javascript">console.log("completed!")</script>';
 $url = 'http://120.55.160.225/api/3rb/traning-lesson-orders';
 $data = array('lessonCodes' => "['0001']", 'username' => 'thunder1');
 $options = array(
    'http' => array(
...
add_action( 'woocommerce_order_edit_status', 'rfvc_update_order_status', 10, 2);
function my_enqueue($hook) {
    if ( 'edit.php' != $hook ) {
        return;
    }

    wp_enqueue_script( 'my_custom_script', get_template_directory_uri() . '/syncOrder.js' );
}
...
$osc_post_types = array(
    array('post_type' => "webinars",
        ...
        'slug' => "webinars",
        'taxonomy' => "cat_webinar",
        'taxonomy_label' => 'Category',
        'taxonomy_singular_label' => 'Webinar Category',
        'taxonomy_slug' => 'webinar',
        'menu_icon' => TEMPLATE_PATH . "/assets/images/icon-webinars.png"),
    array('post_type' => "whitepapers",
        ...
        'not_found' => 'No Whitepaper found',
        'not_found_in_trash' => 'No Whitepaper found in Trash',
        'slug' => "whitepapers",
        'taxonomy' => "cat_whitepaper",
        'taxonomy_label' => 'Category',
        'taxonomy_singular_label' => 'Whitepaper Category',
        'taxonomy_slug' => 'whitepaper',
        'menu_icon' => TEMPLATE_PATH . "/assets/images/icon-whitepapers.png"),
);
require('includes/cpt-webinars.php');
require('includes/cpt-whitepapers.php');
