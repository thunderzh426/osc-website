			...
			case 'order_actions' :
				...
				?><p>
					<?php
						...
						$product_name = array();
						foreach( $order_item as $item_key => $item ) {
							$product = $order->get_product_from_item( $item );
							$sku = $product->get_sku();
							$productID = "'" . $sku . "'";
							$product_name[] = $productID;
            					}
						$product_ids = "[" . implode(',',$product_name) . "]";
						$total = $order->get_total();
						foreach ( $actions as $action ) {
							printf( '<a class="button tips %s" href="%s" data-tip="%s" onclick="syncOrder(%s,\'%s\',\'%s\',%s,%s)">%s</a>', esc_attr( $action['action'] ), esc_url( $action['url'] ), esc_attr( $action['name'] ), $orderId, esc_attr( $action['action'] ), $customer->user_login, $product_ids, $total, esc_attr( $action['name'] ) );
						}
